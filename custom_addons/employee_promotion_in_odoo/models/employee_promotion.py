...
class EmployeePromotion(models.Model):
    ...
    def create(self, vals):
        ...
        contract = self.env['hr.contract.history'].search(
            [('employee_id', '=', self.employee_id.id),
             ('state', '=', 'open')])
        if res.contract_id:
            res.contract_id.write({'wage': res.new_salary})
        return res
    ...
    def write(self, vals):
        ...
        old_salary = self.new_salary
        old_designation = self.new_designation_id
        res = super(EmployeePromotion, self).write(vals)
        if self.job_salary >= self.new_salary:
            raise UserError("New Salary will be Higher than Old ")
        if self.contract_id and old_salary != self.new_salary:
            self.contract_id.write({'wage': self.new_salary})
        if self.contract_id:
            self.contract_id.write({'job_id': self.new_designation_id})
        return res
